...
from cs336_basics.model import (Linear, Embedding, RMSNorm, PWFF,
                RoPE, softmax, scaled_dot_product_attention,
                multihead_self_attention, transformer_block,
                TransformerLM)
from cs336_basics.losses import (cross_entropy, AdamW, learning_rate_schedule,
        gradient_clipping, data_loading,load_checkpoint,save_checkpoint)
...
def run_linear(
    ...
    """
    weight_obj = Linear(d_in,d_out)
    state = {"weight": weights}
    weight_obj.load_state_dict(state)
    return weight_obj(in_features)
...
def run_embedding(
    ...
    embedding_obj = Embedding(vocab_size,d_model)
    state = {"weight": weights}
    embedding_obj.load_state_dict(state)
    return embedding_obj(token_ids)
...
def run_swiglu(
    ...
    weights_obj = PWFF(d_model,d_ff)
    state = {"w1.weight": w1_weight, "w2.weight": w2_weight, "w3.weight": w3_weight}
    weights_obj.load_state_dict(state)

    return weights_obj(in_features)
...
def run_multihead_self_attention(
    ...
    """
    multi_head_attention_obj = multihead_self_attention(
        d_model,num_heads)
    state = {"wq.weight": q_proj_weight,
             "wk.weight": k_proj_weight,
             "wv.weight": v_proj_weight,
             "wo.weight": o_proj_weight}
    multi_head_attention_obj.load_state_dict(state)

    return multi_head_attention_obj(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    return multi_head_attention_obj(in_features,token_positions)
...
def run_rope(
    ...
    """
    rope_obj = RoPE(theta,d_k,max_seq_len)
    return rope_obj(in_query_or_key,token_positions)
...
def run_rmsnorm(
    ...
    """
    rmsnorm_obj = RMSNorm(d_model,eps)
    status = {"g": weights}
    rmsnorm_obj.load_state_dict(status)
    return rmsnorm_obj(in_features)
